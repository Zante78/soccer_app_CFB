-- First clean up all existing objects
DO $$ 
BEGIN
    ...
    DROP FUNCTION IF EXISTS get_player_base_stats(uuid);
    DROP FUNCTION IF EXISTS search_players(text,text);
    DROP FUNCTION IF EXISTS refresh_player_statistics();
    
    -- Drop views if they exist
    DROP VIEW IF EXISTS player_statistics CASCADE;
    DROP MATERIALIZED VIEW IF EXISTS player_statistics CASCADE;
EXCEPTION
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RAISE WARNING 'Error in get_player_stats: %', SQLERRM;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Log error and return empty result
    RAISE WARNING 'Error in search_players: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Create index for active memberships without using CURRENT_DATE
