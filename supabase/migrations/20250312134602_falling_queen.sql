...
-- Create simplified storage policies that work with the current setup
...
WITH CHECK (
  bucket_id = 'teams' AND
  (
    -- Allow upload if user is team owner or admin/coach/manager
    EXISTS (
      SELECT 1 FROM teams
...
WITH CHECK (
  bucket_id = 'teams' AND
  (
    -- Allow update if user is team owner or admin/coach/manager
    EXISTS (
      SELECT 1 FROM teams
      WHERE id::text = split_part(name, '-', 1)
      AND (
...
USING (
  bucket_id = 'teams' AND
  (
    -- Allow delete if user is team owner or admin/coach/manager
    EXISTS (
      SELECT 1 FROM teams
      WHERE id::text = split_part(name, '-', 1)
      AND (
